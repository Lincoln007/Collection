...
{
    /// <summary>
    /// ShopProductOnePageCollector
    /// </summary>
    public class ShopProductOnePageCollector: WebRequestCollector<IResut, NormalParameter>
    {
        ...
        internal static void Test()
        {
            ...
            {
                Keyword = @"http://shop103523226.taobao.com"
            };

            TestHelp<ShopProductOnePageCollector>(parameter);
        }
        ...
        /// 构造函数
        /// </summary>
        public ShopProductOnePageCollector()
        {
                ...
                if (attributes.Contains(@"\""item") && attributes.Contains(@"line1\"""))
                {
                    ...
                    foreach (var htmlNodeDl in htmlNodeDls)
                    {
                        ...
                        if (!_hashTable.ContainsKey(productId))
                        {
                            ...
                            var price = htmlNodeDl.SelectSingleNode(@".//span[@class='\""c-price\""']").InnerText.Trim();
                            var maxPrice = htmlNodeDl.SelectSingleNode(@".//span[@class='\""s-price\""']")?.InnerText.Trim();
                            var saleNum = htmlNodeDl.SelectSingleNode(@".//span[@class='\""sale-num\""']").InnerText.Trim();
                            var comment = Regex.Match(htmlNodeDl.SelectSingleNode(@".//div[@class='\""title\""']").InnerText,@"\d+").Value;
                            var resut = new Resut()
                            {
                                ["productId"] = productId,
